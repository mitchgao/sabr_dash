...
from lib.sabr_model import black
import numpy as np
...
class BaseSABR(metaclass=ABCMeta):
    ...
    def __init__(
        self, f=0.01, shift=0.0, t=1.0, v_atm_n=0.0010, beta=1.0, rho=0.0, volvol=0.0
    ):
        self.f = f
        self.t = t
        self.shift = shift
...
class BaseLognormalSABR(BaseSABR):
    ...
    def call(self, k, cp="call"):
        ...
        f, s, t = self.f, self.shift, self.t
        v_sln = self.lognormal_vol(k)
        pv = black.shifted_lognormal_call(k, f, s, t, v_sln, 0.0, cp)
        return pv
...
class BaseNormalSABR(BaseSABR):
    ...
    def call(self, k, cp="call"):
        ...
        pv = black.normal_call(k, f, t, v_n, 0.0, cp)
        return pv
